...
// 逐层查找define变量
function getDefine(tar, node) {
  var name = node.object.name
  if (name in tar.__define) {
    var def = tar.__define[name]
    var result = this[def.type](def)
    if (result.stats !== STATS.SUCCESS) {
      // at where the #define is called
      result.stack.push(common.getRealLoc([tar, node.pos]))
    }
    return result
  }
  if(!tar.__parent) return
  return getDefine.call(this, tar.__parent, node)
}
...
module.exports = {
  ...
  Reference: function(node) {
    ...
    // call define
    if (common.isId(node)) {
      res = getDefine.call(this, this.template, node)
    }
    if(res) return res
    var obj = node.object
